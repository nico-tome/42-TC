...
void	ft_move_index_to_top(int idx, t_stack *stack_a, t_stack *stack_b)
{
	...
	while (ft_check_combined(stack_a, stack_b, idx, next_value) > 0)
	{
		...
		else if (ft_check_combined(stack_a, stack_b, idx, next_value) == 2)
		{
			ft_rrr(stack_a, stack_b);
			idx = (idx + 1) % stack_a->size;
			next_value = (next_value + 1) % stack_b->size;
		}
	}
	while (next_value != 0 || idx != 0)
		ft_do_move(stack_a, stack_b, &idx, &next_value);
}
