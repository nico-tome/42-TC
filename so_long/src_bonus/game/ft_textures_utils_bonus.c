...
/*   Updated: 2025/11/20 14:21:19 by ntome            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes_bonus/ft_so_long_bonus.h"

void	ft_new_texture(t_mlx *mlx, t_texture *texture, char **paths, int count)
{
	mlx_image	*image;
	int			width;
	int			height;
	...
	i = 0;
	image = malloc(sizeof(mlx_image) * count);
	if (image)
	{
	while (i < count)
	{
		image[i] = mlx_new_image_from_file(mlx->mlx, paths[i], &width, &height);
		i++;
	}
		texture->image = image;
